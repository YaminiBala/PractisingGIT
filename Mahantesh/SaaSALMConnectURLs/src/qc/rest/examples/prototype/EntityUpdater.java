...
public class EntityUpdater {
   ...
   // System.out.println("entityUpdateXml"+entityUpdateXml);
    ...
    private void readAttachment(String instanceId, byte[] filedata, String filename) throws Exception {
        String newEntityToUpdateUrl = this.con.buildEntityUrl("run", "1627811");
        String string = this.lock(newEntityToUpdateUrl);
        Response updateResponseEntityXml = this.readWithOctetStream("1627811", filedata, filename);
        System.out.println("updateResponseEntityXml:" + updateResponseEntityXml);
        boolean bl = this.unlock(newEntityToUpdateUrl);
    }
    ...
    public void CreateTCAttachment(String instanceId, byte[] filedata, String filename) {
     ...
     public void readTCAttachment(String instanceId, byte[] filedata, String filename) {
        try {
            this.readAttachment(instanceId.trim(), filedata, filename.trim());
        }
        catch (Exception e) {
            System.out.println("Exception in Update - Unlock Enitty.....");
            String newEntityToUpdateUrl = this.con.buildEntityUrl("run", instanceId);
    ...
    public String readAttachmentData(String instanceId, byte[] bytes, String attachmentFileName) throws Exception {
        String entityUrl = "http://" + Constants.HOST + ":" + Constants.PORT + "/qcbin/rest/domains/" + Constants.DOMAIN + "/projects/" + Constants.PROJECT + "/resources/" + 1002;
        HashMap<String, String> requestHeaders = new HashMap<String, String>();
        requestHeaders.put("Content-Type", "application/octet-stream");
        requestHeaders.put("Accept", "application/xml");
        byte[] ret = this.con.httpPut(String.valueOf(entityUrl), bytes, requestHeaders).getResponseData();
       con.downloadFile(entityUrl, "C:/ALMConfig");
        return new String(ret);
    }
    ...
    public Response attachWithOctetStream(String Instanceid, byte[] fileData, String filename) throws Exception {
     ...
     public Response readWithOctetStream(String Instanceid, byte[] fileData, String filename) throws Exception {
        String entityUrl = "http://" + Constants.HOST + ":" + Constants.PORT + "/qcbin/rest/domains/" + Constants.DOMAIN + "/projects/" + Constants.PROJECT + "/runs/" + Instanceid;
        HashMap<String, String> requestHeaders = new HashMap<String, String>();
       // requestHeaders.put("Slug", filename);
        requestHeaders.put("Content-Type", "application/octet-stream");
        requestHeaders.put("Accept", "application/xml");
        requestHeaders.put("Content-Disposition", "Attachment; Filename=Log_2016-04-26_12hrs41min20sec.txt");
        Response response = this.con.checkhttpGet(String.valueOf(entityUrl) + "/attachments", "Log_2016-04-26_12hrs41min20sec.txt", requestHeaders);
        
      //  con.downloadFile("http://10.25.130.162:8080/qcbin/rest/domains/CET/projects/ENTERPRISE_SERVICES/runs/1627811/attachments", "C:/ALMConfig");
